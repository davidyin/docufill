...
-- Run: wrangler d1 execute docufill-db --file=src/db/schema.ts
...
-- Extracted fields: structured data pulled from documents
...
-- Extraction results: raw JSON from Gemini (for debugging/reprocessing)
...
-- Form templates: reusable templates for different document types
CREATE TABLE IF NOT EXISTS form_templates (
  id TEXT PRIMARY KEY,
  name TEXT NOT NULL,
  description TEXT,
  category TEXT NOT NULL DEFAULT 'generic' CHECK(category IN ('tax', 'immigration', 'receipt', 'invoice', 'generic')),
  fields_schema TEXT NOT NULL, -- JSON array of {name, label, type, required, validation}
  template_html TEXT,
